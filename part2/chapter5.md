...
实时协作编辑应用允许多个用户同时对同一个文档进行编辑。比如Etherpad[[30](#ch5References30)]和Google Docs[[31](#ch5References31)]就支持多人并发的编辑同一个文本文档或表格（我们会下面[[并发冲突解决](#并发冲突解决)]详细讨论）。
...
多主复制面临的最大问题就是会频繁的产生写冲突，这也就意味着我们必须要解决写冲突。
