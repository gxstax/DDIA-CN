...
3. 你可以处理一个或多个输入流来生成一个或多个输出流。数据流可能经过由多个此类处理阶段组成的管道，最终到达输出端（选项1或2）。

在本章其余的部分，我们会讨论第三种情况：处理流来产生额外的派生流。处理数据流的代码片段就像我们所知道的操作符或作业。它和我们在[第十章](#chapter10.md)中讨论的Unix进程和MapReduce作业紧密关联，数据流模式也类似：
流处理器以只读的方式消费输入流，并以追加的方式将它们输出到不同的地方。

流处理中的分区和并行模式也非常类似于我们在[第十章](#chapter10.md)那些MapReduce和数据流引擎，所以我们在这里就不重复讨论这些话题了。诸如记录的转换和过滤这些基础映射操作工作形式也类似。

相对于批处理作业的唯一关键区别在于流处理永远不会终止。这个区别有多重含义：就像在本章开始时讨论的，对一个无界的数据集进行排序是没有意义的，所以无法使用排序-合并连接（详见：[Reduce端连接和分组](#chapter10.md#Reduce端连接和分组)）。容错机制也必须要改动：对于只运行几分钟的批处理作业，一个失败的任务只需要重启然后从头开始就可以了，但是对于流处理任务，可能已经持续运行了几年了，那么崩溃后从头开始就变的不那么可行了。

### 流处理应用

流处理长期以来被当作监控目的来使用，当组织内部需要对特定事件的发生发出告警。例如：

* 欺诈检测系统需实时监测信用卡使用模式的异常变化，一旦发现疑似被盗交易，立即冻结该卡。

* 交易系统需实时监测金融市场价格波动，并依据既定规则执行交易操作。

* 制造系统需要实时监控工厂内机器的运行状态，一旦发现故障，必须能快速定位问题根源。

* 军事与情报系统需实时追踪潜在侵略者的活动轨迹，一旦发现攻击迹象即刻触发警报。

此类应用需要相当复杂的模式匹配与相关性分析。然而，流处理的其他用途也随着时间的推移逐渐显现。本节将简要比较并对比其中部分应用。

#### 复杂事件处理

复杂事件处理（CEP）是20世纪90年代开发的一种用于分析事件流的方法，尤其适用于需要搜索特定事件模式的应用\[[65](#ch11References65),[66](#ch11References66)]。类似于正则表达式允许你在字符串中搜索特定字符模式的方式，CEP允许你指定规则来搜索事件流中的特定模式。

CEP系统通常采用SQL等高级声明式查询语言或图形用户界面来描述需检测的事件模式。这些查询被提交至处理引擎，该引擎处理输入流并内部维护状态机以执行所需匹配。当匹配成功时，引擎会生成包含所检测事件模式详细信息的复杂事件（因此得名）\[[67](#ch11References67)]。
...
流处理技术的另一应用领域是流式分析。虽然CEP（事件处理）与流分析的界限并不分明，但通常来说，分析工作更侧重于对海量事件进行聚合与统计指标的计算，而非寻找特定事件序列——例如：

* 测量某种事件的发生率（即其在特定时间间隔内的发生频率）
* 计算某段时间内数值的滚动平均值
* 将当前统计数据与既往时间段进行比较（例如，用于检测趋势或对与上周同一时间点相比异常升高或降低的指标发出警报）

这类统计数据通常按固定时间间隔进行计算——例如，您可能需要了解过去5分钟内每秒服务的平均查询量，以及该时段内99百分位响应时间。通过取几分钟的平均值，可以平滑掉秒级流量的无意义波动，同时还能及时捕捉流量模式的变化。这种聚合的时间段称为“时间窗口”，我们将在[“时间推演”](#时间推演)章节中详细探讨窗口机制。
